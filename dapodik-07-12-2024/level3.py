...
df = pd.DataFrame({'link': linkArray, 'nama Sekolah':nmSekolahArray,'NPSN':npsnArray, 'BP':bpArray, 'status':statusArray, 'kode kecamatan':kodeWilayah2,'kecamatan':kecamatanArray, 'kode kab/kota':kodeWilayah1, 'kab/kota': namaWilayah1})

# writer = pd.ExcelWriter('dapodik-level-3.xlsx')
# df.to_excel(writer,'Sheet1',index=False)
df.to_excel('dapodik-level-3.xlsx', index=False, sheet_name='Sheet1')
